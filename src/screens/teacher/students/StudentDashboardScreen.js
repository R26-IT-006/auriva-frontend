...
// All cards take their accent from the child's avatar theme rather than
...
// ── Geometry ──────────────────────────────────────────────────────────────────
/** Avatar at the centre, modules spaced evenly around it — no implied order. */
function buildHub(width, height) {
  ...
  const cards = MODULES.map((m, i) => {
    ...
    return { ...m, x: cx + Math.cos(angle) * dx, y: cy + Math.sin(angle) * dy };
  });
