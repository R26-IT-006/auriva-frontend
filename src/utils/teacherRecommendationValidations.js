 ...
 *
 * ── actionId (Feature 9 repair, final integration audit finding) ───────
 * `submitTeacherRecommendationValidation()` requires an `actionId` from its
 * caller — it never generates one itself. The caller (TeacherReportScreen's
 * TeacherReviewSection) generates exactly one UUID per button press and
 * passes it through here unchanged; this file's job is only to forward it
 * in the request body. This split matters because `client.js`'s response
 * interceptor silently retries a pure network failure by resending the
 * SAME request config (same body, same actionId) — generating a new
 * actionId inside this function would break that retry's idempotency by
 * turning it into what looks like a brand-new action server-side.
 */
 ...
 * `{caseType, family, validation, teacherNote, recommendationFingerprint, actionId}`
 ...
 *
 * `actionId` (Feature 9 repair) is REQUIRED and must be generated by the
 * caller ONCE per button press — this function never generates or
 * regenerates it (see module header). A missing/malformed actionId is
 * rejected server-side as `invalid_input`, same as any other malformed
 * field — this file does not pre-validate it client-side, matching this
 * file's existing convention for every other field.
 *
 * @param {{studentId: number, caseType: string, family: string, validation: 'confirmed'|'dismissed', teacherNote?: string, recommendationFingerprint: string, actionId: string}} params
...
export async function submitTeacherRecommendationValidation({
  studentId, caseType, family, validation, teacherNote, recommendationFingerprint, actionId,
} = {}) {
  const body = {
    caseType,
    family,
