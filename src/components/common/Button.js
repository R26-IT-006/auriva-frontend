...
import { playClickSound } from "./ButtonFeedback";
...
export function Button({
  ...
  const bgColor = pressed.interpolate({
    inputRange: [0, 1],
    outputRange: [cfg.bg || "transparent", cfg.bgPress || "transparent"],
  });
  const txtColor = pressed.interpolate({
    inputRange: [0, 1],
    outputRange: [cfg.textColor, cfg.textPress || cfg.textColor],
  });
      ...
      >
        ...
        >
          {loading ? (
            <ActivityIndicator
              color={
                variant === "danger" ? Colors.status.error : Colors.primary
              }
              size="small"
            />
          ) : (
